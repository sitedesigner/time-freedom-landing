# GoTech.ai Booking Workflow for GHL

## Goal
Only qualified leads receive a GoTech.ai booking link. Everyone else goes to a nurture sequence.

## Step 1: Qualification Fields in GHL
Already captured by landing page form and sent as tags:
- Revenue:250k-500k
- Hours:60-80

## Step 2: Trigger in GHL
When a contact is created via the landing page API at `/api/submit`, the tags come in as:
...
## Step 4: GoTech.ai Booking Form Questions
The confirmation page asks 3 questions. The GoTech.ai booking form should ask:
...
- Use GHL native workflow automation for branching by tags
- Endpoint `/api/ghl-contact-test` can create test contacts with revenue/hours tags
- GoTech.ai booking form should post back to GHL or notify `/api/booking-webhook` on the landing server
